...
def get_language_by_extension(file_path):
    extension_to_language = {
        ...
        '.cs': 'csharp',
        '.swift': 'swift',
        '.ipynb': 'json'  # Jupyter Notebooks are JSON format, adjust if necessary
    }
...
def setup_gradio_interface():
    with gr.Blocks() as interface:
        ...
        with gr.Row():
            ...
            file_extensions_input = gr.Textbox(label="File Extensions (comma-separated)", value=".c, .cpp, .h, .java, .py, .rs, .go, .js, .ts, .rb, .php, .cs, .csproj, .ipynb, .kt, .swift")
        ...
        def merge_to_markdown(_, file_extensions):
            _, downloaded_files = extract_code_files(_, file_extensions)
            if downloaded_files:
