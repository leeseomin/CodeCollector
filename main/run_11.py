...
def clear_download_folder():
    """
    "down_code" 폴더 안의 내용을 모두 삭제하는 함수
    """
...
def extract_code_files(repo_url, file_extensions):
    """
    GitHub 리포지토리에서 지정된 확장자의 코드 파일을 추출하는 함수
    
    :param repo_url: GitHub 리포지토리 URL
    :param file_extensions: 추출할 파일 확장자 리스트
    :return: 추출 결과 메시지와 추출된 파일 리스트
    """
    clear_download_folder()  # "down_code" 폴더 안의 내용을 모두 삭제
...
def combine_code_files_to_markdown(repo_url, downloaded_files):
    """
    추출된 코드 파일들을 하나의 Markdown 파일로 병합하는 함수
    
    :param repo_url: GitHub 리포지토리 URL
    :param downloaded_files: 추출된 파일 리스트
    :return: 병합된 Markdown 파일 경로
    """
...
def setup_gradio_interface():
    ...
    with gr.Blocks() as interface:
        ...
        def merge_to_markdown(repo_url, file_extensions):
            ...
            if downloaded_files:
                return combine_code_files_to_markdown(repo_url, downloaded_files)
